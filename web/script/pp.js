...
app.controller('MyCtrl', function($scope, $http){
  ...
  $scope.doSomething = function(typedthings, user){
	...
	if(typedthings.length > 0)
    {
		...
		if(!user)
			temp = "http://localhost:25809/search?query="+typedthings+"&ranker=comprehensive"
		else if(user.length <= 0)
			temp = "http://localhost:25809/search?query="+typedthings+"&ranker=comprehensive" 
		else
			temp = "http://localhost:25809/search?query="+typedthings+"&username="+user+"&ranker=comprehensive"
